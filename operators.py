...
class EL_OT_record_edit(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        warnings = []
        if layer.data:
            _apply_layer(bm, idl, json.loads(layer.data), warnings, layer, stack, ignore_mix_factor=True)
        bm.normal_update()
        bm.to_mesh(obj.data)
        bm.free()
        obj.data.update()
    ...
    def execute(self, context):
        ...
        try:
            ...
            for l in path[: pos - 1]:
                if l.data:
                    _apply_layer(
                        bm, _ensure_id_layer(bm), json.loads(l.data), warnings, l, stack)
...
class EL_OT_bake(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        _bake_mesh(context.object)
        
        self.report({"INFO"}, _T("Baked and removed the stack"))
        return {"FINISHED"}

class EL_OT_bake_copy(bpy.types.Operator):
    """Apply the active branch result and remove the stack"""

    bl_idname = "edit_layers.bake_copy"
    ...
    def execute(self, context):
        ...
        self.report({"INFO"}, _T("Duplicated with baked Stack"))
        return {"FINISHED"}
